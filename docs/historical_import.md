...
```powershell
& $env:MMA_AI_PYTHON scripts\import_historical_fights.py --source-file ufc_predictor\data\raw\fights.csv --limit 100
```

## Full Import

```powershell
& $env:MMA_AI_PYTHON scripts\import_historical_fights.py --source-file ufc_predictor\data\raw\fights.csv
```
